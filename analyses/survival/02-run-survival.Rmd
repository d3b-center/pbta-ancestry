...
plot_names <- c("ATRT", "CRANIO", "DMG", "HGG", "EPN", "GERM", "GNG", "LGG", "MB", "MNG", "MES", "NFP", "OLIGO", "SWN")
...
for (var in variable){
  ...
  for (group in groups){
    ...
    coxph_os_model_file <- ifelse(group %in% c("Low-grade glioma", "Atypical Teratoid Rhabdoid Tumor",
                                               "Other high-grade glioma", "Schwannoma"),
    ...
    add_model_os <- fit_save_model(group_hist[!grepl("To be classified", group_hist$mol_sub_group),],
                                    terms = ifelse(
                                        group %in% c("Low-grade glioma", "Atypical Teratoid Rhabdoid Tumor"),
...
for (var in variable){
  ...
  for (i in 1:nrow(subtype_df)) {
    ...
    coxph_os_model_file <- ifelse(subtype_df$hist[i] %in% c("LGG", "HGG"),
                               file.path(results_dir,
    ...
    coxph_efs_model_file <- ifelse(subtype_df$hist[i] %in% c("LGG", "HGG"),
                               file.path(results_dir,
    ...
    add_model_os <- fit_save_model(subtype_hist,
                                  terms = ifelse(subtype_df$plot_group[i] == "Low-grade glioma",
                                                 glue::glue("extent_of_tumor_resection+age_at_diagnosis_years+{var}"),
