...
metadata <- metadata %>%
  filter(race %in% c("Not Reported", "Reported Unknown", "Other", "More Than One Race")) %>%
    ...
    plot_group %in% c("Low-grade glioma") & grepl("SEGA", OPC_methyl_subtype) ~ "SEGA",
    plot_group %in% c("Low-grade glioma") & grepl("-BRAF", OPC_methyl_subtype) ~ "LGG, BRAF fusion",
    plot_group %in% c("Low-grade glioma") & grepl("wildtype", OPC_methyl_subtype) ~ "LGG, wildtype",
    ...
    OPC_methyl_subtype == "Atypical Teratoid Rhabdoid Tumor" ~ "ATRT, To be classified",
...
for (group in groups){
  ...
  if(group == "Mixed neuronal-glial tumor|Low-grade glioma") {
    group_hist <- group_hist %>%
  ...
  add_model_os_anc <- fit_save_model(group_hist[!grepl("EPN, H3 K28|EWS|classified|CNS BCOR", group_hist$OPC_methyl_subtype),],
                                  terms = ifelse(
                                      ...
                                      "mol_sub_group+extent_of_tumor_resection+age_at_diagnosis_years+predicted_ancestry",
                                         "mol_sub_group+age_at_diagnosis_years+predicted_ancestry"),
  ...
  add_model_os_eur <- fit_save_model(group_hist,
                                  terms = ifelse(
                                      group %in% c("Mixed neuronal-glial tumor|Low-grade glioma"),
                                      "mol_sub_group+extent_of_tumor_resection+age_at_diagnosis_years+EUR_status",
                                         "mol_sub_group+age_at_diagnosis_years+EUR_status"),
...
metadata <- metadata %>%
  ...
  mutate(mol_sub_group = case_when(
    mol_sub_group == "Atypical Teratoid Rhabdoid Tumor" ~ "ATRT, To be classified",
    TRUE ~ mol_sub_group
  )) %>%
  dplyr::mutate(predicted_ancestry = factor(predicted_ancestry, 
                                            c("EUR", "AFR", "AMR", "EAS", "SAS")))
...
```{r}
  ...
  ggsave(file.path(plots_dir, "km_all_histologies_OS_EFS_predicted_ancestry_unk_race.pdf"),
         km_plot,
  ...
  km_plot <- plotKM(model = list(kap_os_eur, kap_efs_eur),
    ...
    ggsave(file.path(plots_dir, "km_all_histologies_OS_EFS_EURstatus_unk_race.pdf"),
           km_plot,
         width = 10, height = 7, units = "in",
         device = "pdf")


    add_model_os_eur <- fit_save_model(metadata[!grepl("CNS|CHDM|EWS|Other tumor", metadata$mol_sub_group) & metadata$extent_of_tumor_resection != "Not Reported",],
                                      terms = "mol_sub_group+extent_of_tumor_resection+age_at_diagnosis_years+EUR_status",
             file.path(results_dir, "coxph_all_histologies_OS_additive_subtype_EUR_nonEUR_raceUK.RDS"),
             "multivariate",
             years_col = "OS_years",
             status_col = "OS_status"
            )
    
    forest_plot <- plotForest(readRDS(file.path(results_dir, "coxph_all_histologies_OS_additive_subtype_EUR_nonEUR_raceUK.RDS")))
    
        ggsave(file.path(plots_dir, "forest_all_histologies_OS_subtype_EURstatus_unk_race.pdf"),
           forest_plot,
         width = 10, height = 6, units = "in",
         device = "pdf")
        
        
            add_model_efs_eur <- fit_save_model(metadata[!grepl("CNS|CHDM|EWS|Other tumor", metadata$mol_sub_group) & metadata$extent_of_tumor_resection != "Not Reported",],
                                      terms = "mol_sub_group+extent_of_tumor_resection+age_at_diagnosis_years+EUR_status",
             file.path(results_dir, "coxph_all_histologies_EFS_additive_subtype_EUR_nonEUR_raceUK.RDS"),
             "multivariate",
             years_col = "EFS_years",
             status_col = "EFS_status"
            )
    
    forest_plot <- plotForest(readRDS(file.path(results_dir, "coxph_all_histologies_EFS_additive_subtype_EUR_nonEUR_raceUK.RDS")))
    
        ggsave(file.path(plots_dir, "forest_all_histologies_EFS_subtype_EURstatus_unk_race.pdf"),
           forest_plot,
         width = 10, height = 6, units = "in",
         device = "pdf")
